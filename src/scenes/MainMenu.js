...
export default class MainMenu extends Phaser.Scene {
  ...
  _drawTitle() {
    ...
    this.add.text(W / 2, 130, 'BLAST', {
      ...
      stroke: '#6c63ff', strokeThickness: 8,
      shadow: { offsetX: 4, offsetY: 4, color: '#000', blur: 16, fill: true },
    }).setOrigin(0.5);

    this.add.text(W / 2, 215, t('subtitle'), {
      fontFamily: 'Arial, sans-serif', fontSize: '28px',
      color: '#ce93d8', letterSpacing: 10,
    }).setOrigin(0.5);

    this.tweens.add({
      targets: glow, scaleX: 1.04, scaleY: 1.04,
  ...
  _drawPlayBtn() {
    ...
    btn.fillRoundedRect(W/2 - 120, btnY - 28, 240, 56, 28);
    btn.setInteractive(new Phaser.Geom.Rectangle(W/2-120, btnY-28, 240, 56), Phaser.Geom.Rectangle.Contains);

    const label = this.add.text(W / 2, btnY, t('play'), {
      fontFamily: 'Arial, sans-serif', fontSize: '26px',
      fontStyle: 'bold', color: '#ffffff',
    }).setOrigin(0.5);

    btn.on('pointerover', () => { btn.clear(); btn.fillStyle(0x9c8fff,1); btn.fillRoundedRect(W/2-120,btnY-28,240,56,28); });
    btn.on('pointerout',  () => { btn.clear(); btn.fillStyle(0x6c63ff,1); btn.fillRoundedRect(W/2-120,btnY-28,240,56,28); });
    btn.on('pointerdown', () => {
    ...
    this.tweens.add({
      targets: [btn, label], scaleX: 1.04, scaleY: 1.04,
  ...
  _drawWorldPreviews() {
    ...
    this.add.text(W / 2, startY - 20, t('worldsPreview'), {
      fontFamily: 'Arial, sans-serif', fontSize: '16px',
      color: '#aaaacc', letterSpacing: 4,
    }).setOrigin(0.5);
    ...
    WORLDS.forEach((world, i) => {
      ...
      this.add.text(x + 32, y + 46, t(`world_${i + 1}`), {
        fontFamily: 'Arial, sans-serif', fontSize: '13px', color: '#ddddff',
      }).setOrigin(0.5);
    });
  }

  _drawLangToggle() {
    const btn = this.add.text(W - 16, 16, t('lang'), {
      fontFamily: 'Arial, sans-serif', fontSize: '16px',
      fontStyle: 'bold', color: '#ffffff',
