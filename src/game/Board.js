...
export class Tile {
  ...
  isObstacle()    { return this.type === TYPE.STONE || this.type === TYPE.ICE || this.type === TYPE.LAVA; }
  isMovable()     { return this.type !== TYPE.STONE; }
  isSpecialPower(){ return this.type === TYPE.BOMB || this.type === TYPE.MEGA ||
                          this.type === TYPE.STRIPE_H || this.type === TYPE.STRIPE_V ||
                          this.type === TYPE.RAINBOW; }
  isAutoTrigger() { return this.type === TYPE.RAINBOW || this.type === TYPE.MEGA; }
}
...
export class Board {
  constructor(cols, rows, levelDef) {
    ...
    this.inactiveCells = this._computeShape(levelDef.boardShape);
    this._init();
  }
  ...
  _computeShape(shape) {
    ...
    if (shape === 'corners-1') {
      [[0,0],[0,C-1],[R-1,0],[R-1,C-1]].forEach(([r,c]) => s.add(`${r},${c}`));
    }
    else if (shape === 'corners-2') {
      for (let dr = 0; dr < 2; dr++) for (let dc = 0; dc < 2; dc++) {
        s.add(`${dr},${dc}`);       s.add(`${dr},${C-1-dc}`);
        s.add(`${R-1-dr},${dc}`);   s.add(`${R-1-dr},${C-1-dc}`);
      }
    }
    else if (shape === 'plus') {
      const qr = Math.ceil(R / 4), qc = Math.ceil(C / 4);
      for (let r = 0; r < R; r++) for (let c = 0; c < C; c++)
        if ((r < qr || r >= R-qr) && (c < qc || c >= C-qc)) s.add(`${r},${c}`);
    }
    else if (shape === 'diamond') {
      for (let r = 0; r < R; r++) for (let c = 0; c < C; c++) {
        const nr = R > 1 ? Math.abs(r/(R-1) - 0.5) / 0.5 : 0;
        const nc = C > 1 ? Math.abs(c/(C-1) - 0.5) / 0.5 : 0;
        if (nr + nc > 1.3) s.add(`${r},${c}`);
      }
    }
    return s;
  }

  isInactive(r, c) { return this.inactiveCells.has(`${r},${c}`); }
  ...
  _init() {
    ...
    const place = (set, count, exclude = []) => {
      ...
      while (set.size < count && tries < 1000) {
        ...
        const key = `${r},${c}`;
        if (!this.inactiveCells.has(key) && !exclude.some(s => s.has(key))) set.add(key);
      }
    };

    const stoneCells = new Set(), iceCells = new Set(), lavaCells = new Set();
    ...
    for (let r = 0; r < this.rows; r++) {
      ...
      for (let c = 0; c < this.cols; c++) {
        const key = `${r},${c}`;
        if (this.inactiveCells.has(key)) { this.cells[r][c] = null; continue; }
  ...
  // RAINBOW is a wildcard — matches any non-obstacle tile
  _matchable(a, b) {
  ...
  findMatches() {
    ...
    for (let r = 0; r < this.rows; r++) {
      ...
      while (c < this.cols) {
        const t = this.cells[r][c];
        if (!t || t.isObstacle() || this.isInactive(r, c)) { c++; continue; }
        let len = 1;
        while (c + len < this.cols) {
          const n = this.cells[r][c + len];
          if (n && this._matchable(t, n) && !this.isInactive(r, c+len)) len++;
          else break;
        }
    ...
    for (let c = 0; c < this.cols; c++) {
      ...
      while (r < this.rows) {
        const t = this.cells[r][c];
        if (!t || t.isObstacle() || this.isInactive(r, c)) { r++; continue; }
        let len = 1;
        while (r + len < this.rows) {
          const n = this.cells[r + len][c];
          if (n && this._matchable(t, n) && !this.isInactive(r+len, c)) len++;
          else break;
        }
  ...
  applyMatches(matches, chainLevel = 1) {
    ...
    for (const group of matches) {
      ...
      const pivot   = group.cells[Math.floor(group.cells.length / 2)];
      ...
      for (const {r, c} of group.cells) {
        ...
        if (t?.isSpecialPower()) {
          ...
          if (t.type === TYPE.BOMB || t.type === TYPE.MEGA) bombsTriggered++;
        }
      }

      if (special && pivot) newSpecials.push({
        r: pivot.r, c: pivot.c, type: special,
        color: this.cells[pivot.r][pivot.c]?.color ?? 0,
      });
    ...
    for (const {r, c, type, color} of newSpecials) {
      if (!this.cells[r][c]) this.cells[r][c] = new Tile(color, type);
    }

    return { scoreAdd, icesBroken, deleted: toDelete, colorsCleared, bombsTriggered };
  }

  // ── Force-trigger a special at (r,c) — used for swap-activate ────────────
  triggerAt(r, c) {
    const tile = this.get(r, c);
    if (!tile) return { deleted: new Set(), colorsCleared: new Array(6).fill(0), scoreAdd: 0, icesBroken: 0 };

    const toDelete = new Set([`${r},${c}`]);
    this._triggerSpecial(tile.type, r, c, toDelete);

    // Break adjacent ice for each marked cell
    let icesBroken = 0;
    for (const key of [...toDelete]) {
      const [dr, dc] = key.split(',').map(Number);
      for (const [ddr, ddc] of [[-1,0],[1,0],[0,-1],[0,1]]) {
        const ice = this.get(dr+ddr, dc+ddc);
        if (ice && ice.type === TYPE.ICE) {
          ice.iceHp--;
          if (ice.iceHp <= 0) { toDelete.add(`${dr+ddr},${dc+ddc}`); icesBroken++; }
        }
      }
    }
    ...
    for (const key of toDelete) {
      const [dr, dc] = key.split(',').map(Number);
      const t = this.cells[dr]?.[dc];
      if (!t || t.type === TYPE.STONE) continue;
      if (!t.isObstacle()) colorsCleared[t.color]++;
      scoreAdd += 150;
      this.cells[dr][dc] = null;
    }

    return { deleted: toDelete, colorsCleared, scoreAdd, icesBroken };
  }

  _triggerSpecial(type, r, c, toDelete) {
    const mark = (row, col) => {
      const t = this.get(row, col);
      if (t && t.type !== TYPE.STONE && !this.isInactive(row, col)) toDelete.add(`${row},${col}`);
    };
    ...
    for (let c = 0; c < this.cols; c++) {
      if (!goingUp) {
        ...
        for (let r = this.rows - 1; r >= 0; r--) {
          if (this.isInactive(r, c)) { empty = r - 1; continue; }
          const t = this.cells[r][c];
          if (t && t.isMovable()) {
            if (empty !== r) {
  ...
  fillEmpty(colorCount) {
    ...
    for (let r = 0; r < this.rows; r++)
      for (let c = 0; c < this.cols; c++)
        if (!this.cells[r][c] && !this.isInactive(r, c)) {
